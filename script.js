...
/* ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
   ...
   ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★ */
const API_URL = 'https://script.google.com/macros/s/AKfycbzJL6E9BAra21VxjPdRfB9sg4Xid9Y7QXpDD3tM9Rcjl3p1gZHXBoE5DttjW6B3fRmAdA/exec';
...
async function loadCore(force) {
  ...
  state.links = (data.links || []).sort(byOrder);
...
/* ============================================================================
   10.6) 🔗 เว็บไซต์อื่นๆ (ลิงก์ภายนอก เช่น เว็บโรงเรียน)
   ============================================================================ */
function findLink(id) { return (state.links || []).find(l => l.id === id); }

// โซนเว็บไซต์อื่นๆ บนหน้าแรก (ต่อจากโซนเกม)
function linksSection() {
  const links = (state.links || []).filter(l => isTeacher() || (l.status || 'active') === 'active').sort(byOrder);
  if (!links.length && !isTeacher()) return '';   // ไม่มีลิงก์ + เป็นนักเรียน = ซ่อน

  const head = `<div class="section-head">
      <h2>🔗 เว็บไซต์อื่นๆ</h2>
      <button class="btn btn-outline btn-sm teacher-only" onclick="openLinkForm()">➕ เพิ่มเว็บไซต์</button>
    </div>`;
  if (!links.length) {
    return head + viewEmpty('🔗', 'ยังไม่มีเว็บไซต์', 'กดปุ่ม "เพิ่มเว็บไซต์" เพื่อใส่ลิงก์เว็บโรงเรียนหรือเว็บอื่น ๆ');
  }
  return `${head}<div class="links-grid">${links.map((l, i) => linkCard(l, i)).join('')}</div>`;
}

// การ์ดลิงก์เว็บ 1 ใบ
function linkCard(l, i) {
  const cover = imgUrl(l.cover_image);
  const inactive = (l.status || 'active') !== 'active' ? ' inactive' : '';
  const thumb = cover
    ? `<img src="${esc(cover)}" alt="" loading="lazy" onerror="this.parentNode.classList.add('noimg');this.remove()" />`
    : `<span class="link-emoji">${esc(l.icon || '🔗')}</span>`;
  return `
    <div class="link-card${inactive}" style="animation-delay:${i * 50}ms">
      <a class="link-main" href="${esc(l.url || '#')}" target="_blank" rel="noopener">
        <div class="link-thumb ${cover ? '' : 'noimg'}">${thumb}</div>
        <div class="link-body">
          <div class="link-title">${esc(l.title || 'เว็บไซต์')}</div>
          ${l.description ? `<div class="link-desc">${esc(l.description)}</div>` : ''}
          <div class="link-go">ไปที่เว็บไซต์ ↗</div>
        </div>
      </a>
      <div class="link-tools teacher-only">
        <button class="btn btn-outline btn-sm" onclick="openLinkForm('${esc(l.id)}')">✏️ แก้ไข</button>
        <button class="btn btn-danger btn-sm" onclick="confirmDeleteLink('${esc(l.id)}')">🗑️ ลบ</button>
      </div>
    </div>`;
}

// ฟอร์มเพิ่ม/แก้ไขเว็บไซต์ (เฉพาะครู)
function openLinkForm(id) {
  const l = id ? findLink(id) || {} : {};
  const isEdit = !!id;
  openModal(isEdit ? 'แก้ไขเว็บไซต์' : 'เพิ่มเว็บไซต์', `
    <form id="linkForm" novalidate>
      <div class="field"><label>ชื่อเว็บไซต์ <span class="req">*</span></label><input name="title" value="${esc(l.title || '')}" placeholder="เช่น เว็บไซต์โรงเรียน" /><div class="err">กรุณากรอกชื่อเว็บไซต์</div></div>
      <div class="field"><label>ลิงก์ (URL) <span class="req">*</span></label><input name="url" value="${esc(l.url || '')}" placeholder="https://..." /><div class="err">กรุณากรอกลิงก์ (ขึ้นต้นด้วย http)</div></div>
      <div class="field"><label>คำอธิบายสั้น ๆ</label><textarea name="description" placeholder="เว็บนี้เกี่ยวกับอะไร (ถ้ามี)">${esc(l.description || '')}</textarea></div>
      <div class="field"><label>ไอคอน (อีโมจิ)</label><input name="icon" value="${esc(l.icon || '🔗')}" placeholder="🏫" /><div class="hint">ใช้เมื่อไม่ได้ใส่รูปปก</div></div>
      ${imageField('cover_image', 'รูปปก (ถ้ามี)', l.cover_image)}
      <div class="row2">
        <div class="field"><label>ลำดับการแสดง</label><input type="number" name="order" value="${esc(l.order || ((state.links || []).length + 1))}" /></div>
        <div class="field"><label>การแสดงผล</label><select name="status"><option value="active" ${(l.status||'active')==='active'?'selected':''}>แสดง (เปิด)</option><option value="inactive" ${l.status==='inactive'?'selected':''}>ซ่อน (ปิด)</option></select></div>
      </div>
      <div class="form-actions">
        <button type="button" class="btn btn-outline" onclick="closeModal()">ยกเลิก</button>
        <button type="submit" class="btn btn-primary">${isEdit ? 'บันทึกการแก้ไข' : 'เพิ่มเว็บไซต์'}</button>
      </div>
    </form>
  `);
  wireImageField('cover_image', $('#modalBody'));
  $('#linkForm').addEventListener('submit', async (e) => {
    ...
    f.elements.title.closest('.field').classList.toggle('invalid', !title);
    f.elements.url.closest('.field').classList.toggle('invalid', !urlOk);
    if (!title || !urlOk) return;
    const item = {
      id: l.id || '', title, url,
    ...
    await submitForm(f, () => apiPost('saveLink', { item }), 'บันทึกเว็บไซต์แล้ว', () => { invalidateCache(); renderHome(); });
  });
}

function confirmDeleteLink(id) {
  const l = findLink(id) || {};
  openConfirm('ต้องการลบเว็บไซต์นี้ใช่ไหม?', l.title || '', '', async () => {
